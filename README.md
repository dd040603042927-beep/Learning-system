...
这是一个以“用户自定义目标”为中心的个人学习管理系统。v5 的主题是“智能训练与真实语义知识库”：系统不只记录笔记和计划，还会把外部学习资料转成笔记、知识点、自测题和复习计划，通过 AI 自动批改、低分错题闭环、掌握度更新、每日学习驾驶舱和带来源的知识库问答，把资料管理升级成学习助手。当前版本已经具备本地轻量 embedding、相似度计算和关键词混合检索；后续如果接入真实 embedding API 或向量数据库，可以复用现有搜索文档与排序接口。
...
- AI 自动批改：每道题可以绑定 Rubric，提交答案后生成分数、优点、扣分原因、遗漏知识点、改进版答案和下一步建议。
...
- 知识库：新增资料导入、资料分段、导入任务记录、本地轻量 embedding、关键词混合检索、带来源问答和来源展示。
- 检索能力边界：当前为本地轻量 embedding + 关键词混合检索。系统会为笔记、资料、资料分段、题目、错题、知识点、目标、里程碑和复盘生成搜索文档，并用关键词分与语义相似度综合排序；如果没有可靠来源，知识库问答不会生成肯定答案。
...
- 每日学习驾驶舱：自动生成“今天该学什么”队列，综合目标重要程度、截止日期、到期复习、错题、掌握度、学习路径下一步和里程碑紧急度排序，并支持一键开始、完成和推迟。
...
- `POST /api/resources/extract-text`：从 TXT/Markdown、DOCX 正文和文本型 PDF 中提取资料正文，失败时返回可展示的解析错误。
- `GET /api/health`：健康检查。
